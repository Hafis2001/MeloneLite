...
export default function TakeOrderScreen() {
  ...
  const takeOrderCartCount = getTotalItems();
  const takeOrderCartSubtotal = getSubtotal();
    ...
    >
      <View style={styles.contentWrapper}>
        ...
        <View style={[styles.header, isLandscape && isPhone && styles.headerLandscape]}>
          <View>
            <Text style={[styles.headerTitle, isLandscape && isPhone && { fontSize: 18 }]}>Take Order</Text>
            {!isLandscape && (
          ...
          <View style={styles.headerRight}>
            ...
            {requireBarcode && (
              <TouchableOpacity
                ...
                onPress={() => router.push({ pathname: '/barcode-scanner', params: { mode: 'scan_take-order-cart' } })}
                activeOpacity={0.7}
              >
                <MaterialCommunityIcons name="barcode-scan" size={22} color={Colors.gold} />
              </TouchableOpacity>
            )}
            {/* TakeOrderCart Button */}
            <TouchableOpacity style={styles.headerTakeOrderCartBtn} onPress={() => router.push('/take-order-cart')}>
              <MaterialCommunityIcons name="cart-outline" size={24} color={Colors.gold} />
              {takeOrderCartCount > 0 && (
                <View style={styles.headerBadge}>
                  <Text style={styles.headerBadgeText}>{takeOrderCartCount}</Text>
                </View>
              )}
            </TouchableOpacity>
          </View>
        ...
        {filteredItems.length === 0 ? (
          ...
          <FlatList
            ...
            contentContainerStyle={[styles.listContent, { paddingBottom: takeOrderCartCount > 0 ? 110 : 60 }]}
                  ...
                  <>
                    ...
                    <View style={styles.modalBtns}>
                      ...
                      <TouchableOpacity style={styles.modalAddBtn} onPress={confirmBarcodeAdd}>
                        ...
                        <Text style={styles.modalAddText}>Add to TakeOrderCart</Text>
                      </TouchableOpacity>
                    </View>
                  </>
                ) : null}
...
function createStyles() {
  ...
  takeOrderCartBarLeft: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm },
  takeOrderCartCountBubble: {
    backgroundColor: Colors.textInverse, borderRadius: Radius.full,
    width: 26, height: 26, alignItems: 'center', justifyContent: 'center',
  },
  takeOrderCartCountText: { color: Colors.gold, fontFamily: 'Poppins-Bold', fontSize: 12 },
  takeOrderCartBarLabel: { color: Colors.textInverse, fontFamily: 'Poppins-SemiBold', fontSize: 14 },
  takeOrderCartBarRight: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  takeOrderCartBarTotal: { color: Colors.textInverse, fontFamily: 'Poppins-Bold', fontSize: 16 },
