...
import { useTakeOrderCart } from '../src/context/TakeOrderCartContext';
import { placeTakeOrder, getTakeOrderById } from '../src/db/takeOrdersDB';
...
const CartItemRow = ({ ci, updateQuantity, removeItem, formatCurrency, styles }: any) => {
  ...
  return (
    <View style={styles.TakeOrderCartItem}>
      <View style={styles.TakeOrderCartItemTopRow}>
        <Text style={styles.TakeOrderCartItemName} numberOfLines={1}>
          {ci.item.item_name}
      ...
      <View style={styles.TakeOrderCartItemBottomRow}>
        <Text style={styles.TakeOrderCartItemRate}>{formatCurrency(ci.selectedVariant ? ci.selectedVariant.price : ci.item.rate)}</Text>
        ...
        <Text style={styles.TakeOrderCartItemSubtotal}>{formatCurrency((ci.selectedVariant ? ci.selectedVariant.price : ci.item.rate) * ci.quantity)}</Text>
      </View>
    </View>
  );
};
...
export default function TakeOrderCartScreen() {
  const themeVersion = useThemeVersion();
  const segments = useSegments();
  const isStandalone = segments.includes('take-order-cart');
  ...
  const handleDirectPrint = async (orderId: number) => {
    const full = getTakeOrderById(orderId);
      ...
      Alert.alert(
        ...
        [
          ...
          { 
            ...
            onPress: async () => {
              try {
                await printReceipt(full as any, full.items as any, settings, true);
              } catch (e) {
  ...
  const handlePlaceOrder = async () => {
    if (state.items.length === 0) {
      Alert.alert('Empty TakeOrderCart', 'Add items to TakeOrderCart before placing order');
      return;
    }
    setPlacing(true);
    try {
        ...
        <View style={[styles.header, isLandscapePhone && styles.headerLandscape]}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.sm }}>
            ...
            {state.items.length > 0 && (
              <TouchableOpacity onPress={() => Alert.alert('Clear TakeOrderCart', 'Remove all items?', [
                { text: 'Cancel', style: 'cancel' },
          ...
          {state.items.length === 0 ? (
            ...
            <>
              ...
              <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: Spacing.lg, paddingTop: Spacing.lg }} showsVerticalScrollIndicator={false}>
                ...
                {state.items.map(ci => <CartItemRow key={ci.cartItemId} ci={ci} updateQuantity={updateQuantity} removeItem={removeItem} formatCurrency={formatCurrency} styles={styles} />)}
                </View>
              </ScrollView>
              ...
              <View style={{ paddingHorizontal: Spacing.lg, paddingBottom: Spacing.sm }}>
                ...
                {/* Remark Field */}
                <View style={[styles.section, { paddingVertical: 4, paddingHorizontal: Spacing.md, marginBottom: Spacing.sm }]}>
                  <View style={[styles.inputGroup, { borderBottomWidth: 0 }]}>
                    <MaterialCommunityIcons name="note-edit-outline" size={18} color={Colors.gold} />
                    <TextInput 
          ...
          <View style={styles.modalOverlay}>
            <View style={[styles.modalCard, { alignItems: 'center', paddingVertical: Spacing.xxl }]}>
              ...
              <View style={{ width: '100%', gap: Spacing.sm }}>
                ...
                <TouchableOpacity 
                  style={[styles.modalDoneBtn, { backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.gold, marginTop: 0, marginBottom: Spacing.sm }]} 
                  onPress={async () => {
...
function createStyles() {
  ...
  TakeOrderCartItem: { flexDirection: 'column', marginBottom: Spacing.md, paddingVertical: 4 },
  TakeOrderCartItemTopRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 },
  TakeOrderCartItemBottomRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  TakeOrderCartItemName: { ...Typography.bodyMedium, fontSize: 14, flex: 1, marginRight: 8 },
  TakeOrderCartItemRate: { ...Typography.caption, flex: 1 },
  qtyControls: {
  ...
  TakeOrderCartItemSubtotal: { ...Typography.priceSmall, width: 70, textAlign: 'right' },
