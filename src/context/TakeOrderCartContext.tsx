...
const initialState: TakeOrderCartState = {
  items: [],
  customerName: '',
  tableNo: '',
  paymentMethod: 'Cash',
...
function takeOrderCartReducer(state: TakeOrderCartState, action: TakeOrderCartAction): TakeOrderCartState {
  switch (action.type) {
    case 'ADD_ITEM': {
...
interface TakeOrderCartContextType {
  state: TakeOrderCartState;
...
const TakeOrderCartContext = createContext<TakeOrderCartContextType | null>(null);

export const TakeOrderCartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, dispatch] = useReducer(takeOrderCartReducer, initialState);
  ...
  return (
    <TakeOrderCartContext.Provider value={{
      ...
      {children}
    </TakeOrderCartContext.Provider>
  );
};

export const useTakeOrderCart = (): TakeOrderCartContextType => {
  const ctx = useContext(TakeOrderCartContext);
  if (!ctx) throw new Error('useTakeOrderCart must be used within TakeOrderCartProvider');
  return ctx;
};
